...
def test_simulate_position_choice_threads_custom_opponent_weight_fn_to_sample_pick():
    ...
    with patch("fantasyprep.draft_sim.simulate.OpponentSampler", side_effect=spy_opponent_sampler):
        results = simulate_position_choice(
    ...
    assert results is not None
    assert seen_weight_fns  # OpponentSampler was actually constructed
